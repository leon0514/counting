...
def postprocess(img0, prediction, conf_thres, iou_thres, input_h, input_w, classes=None):
    """
    img0: original image
    prediction: YOLOv8 output and after reshape, default shape is [84, 8400]
    classes: list[int] or None, 只保留指定类别 (如 [0] 只保留 person)
    """
    ...
    if classes is not None:
        labels = labels.intersection(set(classes))
