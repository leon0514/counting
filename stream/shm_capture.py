...
import grpc
try:    
    from stream import stream_service_pb2
    from stream import stream_service_pb2_grpc
except ImportError:
...
class ShmCapture:
    ...
    def __init__(self, server: str = '127.0.0.1:50052'):
        ...
        self.last_frame_timestamp = 0  # C++ 后端写入该帧的时间戳（毫秒级）
    ...
    def read(self, blocking: bool = False, timeout_ms: Optional[float] = None) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        ok, img, ts = self._shm.read(blocking=blocking, timeout_ms=timeout_ms)
